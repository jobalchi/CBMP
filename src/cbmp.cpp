...
CBM_Pred::CBM_Pred() : nh_(), private_nh_("~"), MAX_PREDICTION_SCORE_(1.0){
  ...
  // Load CSV files for each lane
  loadLaneData("/home/jobalchi/Ros1/pred_ws/src/cbmp/map/lane_1.csv", cx_1, cy_1, cyaw_1, ck_1);
  loadLaneData("/home/jobalchi/Ros1/pred_ws/src/cbmp/map/lane_2.csv", cx_2, cy_2, cyaw_2, ck_2);
  loadLaneData("/home/jobalchi/Ros1/pred_ws/src/cbmp/map/lane_3.csv", cx_3, cy_3, cyaw_3, ck_3);
}
...
void CBM_Pred::loadLaneData(const std::string& filename, std::vector<double>& cx, std::vector<double>& cy, std::vector<double>& cyaw, std::vector<double>& ck) {
...
int CBM_Pred::selectClosestLane(double x, double y) {
  double dist1 = calcDistanceToLane(x, y, cx_1, cy_1);
  double dist2 = calcDistanceToLane(x, y, cx_2, cy_2);
  double dist3 = calcDistanceToLane(x, y, cx_3, cy_3);
...
double CBM_Pred::calcDistanceToLane(double x, double y, const std::vector<double>& cx, const std::vector<double>& cy) {
  std::vector<double> d(cx.size());
  for (size_t i = 0; i < cx.size(); i++) {
      d[i] = std::hypot(x - cx[i], y - cy[i]);
  }
...
autoware_msgs::DetectedObject CBM_Pred::controlBasedMotionPrediction(const autoware_msgs::DetectedObject& object){
  ...
  // Select closest lane
  int closest_lane = selectClosestLane(state.x, state.y);
